...
The aim of this project to to predict the manner(classe A,B,C,D,E) in which the 6 healthy young men did their unilateral dumbbell bicep curl exercise.The report used the data collected from accelerometers on the belt, forearm, arm, and dumbell. A model is then built using PCA preProcessing method to narrow down variables and random forest method (with its own cross validation) to predict results. The model got 97.59% accuracy and out of sample error rate of 2.41%. The out of sample error rate comes from the testing data outside of the training data. 
...
**Step1: To input data**,since we've already read and cleansed the data. Here by inputing data, we mean split the data into 70% training and 30% testing using createDataPartition (){caret} function.The split training data is called *train* and split testing data is called *test*. The testing data we loaded becomes validation dataset. 
...
Using the same PCA scheme to preprocess the testing file we originally loaded,we got 20 predictions and write them with the given write.table function to generate 20 txt file in the working directory.Submitting the answers, we got 20 out of 20 predictions correctly.  

#**5.Appendix**

Part1: not shown and only used to generate the charts; Part 2: display all the code together for viewing convenience. 
...
qplot(classe,colour=user_name,data=train,geom="density",main = "Chart 1: Density for Outcome Variable")
...
```{r,echo=FALSE}
df=matrix(c("0.9759","0.0241","< 2.2e-16"," 2.5%"), ncol=4, nrow=1,byrow=TRUE,dimnames=list("value", c("Accuracy","Out of Sample Error Rate", "P-Value ","OOB Error Rate")))
          
knitr::kable(df,format="pandoc",caption = "Chart3: ConfusionMatrix Results: 97.81% Accuracy ")
