...
The aim of this project to to predict the manner(classe A,B,C,D,E) in which the 6 healthy young men did their unilateral dumbbell bicep curl exercise.The report used the data collected from accelerometers on the belt, forearm, arm, and dumbell. A model is then built using PCA preProcessing method to narrow down variables and cross validation to split data as well as random forest method to predict results. The model got 97.35% accuracy with out of sample error of...
...
**To evaluate the result**, using confusionMatrix function, we found 97.35% overll model accuracy and sensitivity and specicity for each type of "classe" is above 90% each. If printing out the model itself, it has OOB(out of bag) error rate of 2.5% which is a very small good rate. The ooB error rate is the out of sample error rate in random forest because of the unique way that random forest operates. 

This tells it's a really good model and proves that random forest as one of the most common CART methods has very high accuracy. However, the downside of using randomForest is that it will overfit a model.The reason is that a randomForest fittd model is normally complex, having many parameters relative to the number of observations. It is expected that the fitted relationship will appear to perform less well on a new data set than on the data set we used for fitting. Therefore, we need to use some techniques to avoid overfitting. 
...
testing0=select(testing0,-starts_with("kurtosis"))
testing0=select(testing0,-starts_with("skewness"))
testing0=select(testing0,-starts_with("max"))
testing0=select(testing0,-starts_with("min"))
testing0=select(testing0,-starts_with("var"))
testing0=select(testing0,-starts_with("stddev"))
testing0=select(testing0,-starts_with("avg"))
